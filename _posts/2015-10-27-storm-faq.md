...
### 配置Storm+Trident的准则是什么？

* 工作进程的个数应该是机器数的整数倍；并行度应该是工作进程个数的整数倍；Kafka分区的个数应该是Spout并行度的整数倍
* 每台机器上只为每个拓扑分配一个工作进程
* Start with fewer, larger aggregators, one per machine with workers on it
* 使用独立的调度器
* 每个工作进程使用一个acker进程——0.9版本将此作为默认设置，但是之前的版本并非如此
* 打开GC日志；如果一切正常，大的GC动作很少会发生
* 将Trident的批处理时间设置为端到端延迟典型值的一半
...
如果记录带有时间戳，而且用户希望根据离散的时间范围来对事件进行计数、求均值或其他的聚合操作，Trident是一个很好的可伸缩的解决方案。
...
### 如何知道某一时间段内一共收到了多少条记录？

用户无法确定所有事件都被收集到了——这是认知论上的一个挑战，而非分布式系统的。用户可以：
